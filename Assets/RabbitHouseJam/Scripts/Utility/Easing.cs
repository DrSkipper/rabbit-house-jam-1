...
public class Easing
{
    ...
    /// <summary>
    /// Reference to an easing function.
    /// </summary>
    /// <param name="t">Current time.</param>
    /// <param name="b">Starting value.</param>
    /// <param name="c">Final easing distance.</param>
    /// <param name="d">Duration of animation.</param>
    /// <returns>The correct value.</returns>
    public delegate float EasingDelegate(float t, float b, float c, float d);

    public static EasingDelegate GetFunction(Function function, Flow flow = Flow.InOut)
    {
        switch (function)
        {
            default:
            case Function.Linear:
                return Linear;
            case Function.Expo:
                if (flow == Flow.Out) return ExpoEaseOut;
                if (flow == Flow.In) return ExpoEaseIn;
                if (flow == Flow.InOut) return ExpoEaseInOut;
                return ExpoEaseOutIn;
            case Function.Circ:
                if (flow == Flow.Out) return CircEaseOut;
                if (flow == Flow.In) return CircEaseIn;
                if (flow == Flow.InOut) return CircEaseInOut;
                return CircEaseOutIn;
            case Function.Quad:
                if (flow == Flow.Out) return QuadEaseOut;
                if (flow == Flow.In) return QuadEaseIn;
                if (flow == Flow.InOut) return QuadEaseInOut;
                return QuadEaseOutIn;
            case Function.Sine:
                if (flow == Flow.Out) return SineEaseOut;
                if (flow == Flow.In) return SineEaseIn;
                if (flow == Flow.InOut) return SineEaseInOut;
                return ExpoEaseOutIn;
            case Function.Cubic:
                if (flow == Flow.Out) return CubicEaseOut;
                if (flow == Flow.In) return CubicEaseIn;
                if (flow == Flow.InOut) return CubicEaseInOut;
                return CubicEaseOutIn;
            case Function.Quart:
                if (flow == Flow.Out) return QuartEaseOut;
                if (flow == Flow.In) return QuartEaseIn;
                if (flow == Flow.InOut) return QuartEaseInOut;
                return QuartEaseOutIn;
            case Function.Quint:
                if (flow == Flow.Out) return QuintEaseOut;
                if (flow == Flow.In) return QuintEaseIn;
                if (flow == Flow.InOut) return QuintEaseInOut;
                return QuintEaseOutIn;
            case Function.Elastic:
                if (flow == Flow.Out) return ElasticEaseOut;
                if (flow == Flow.In) return ElasticEaseIn;
                if (flow == Flow.InOut) return ElasticEaseInOut;
                return ElasticEaseOutIn;
            case Function.Bounce:
                if (flow == Flow.Out) return BounceEaseOut;
                if (flow == Flow.In) return BounceEaseIn;
                if (flow == Flow.InOut) return BounceEaseInOut;
                return BounceEaseOutIn;
            case Function.Back:
                if (flow == Flow.Out) return BackEaseOut;
                if (flow == Flow.In) return BackEaseIn;
                if (flow == Flow.InOut) return BackEaseInOut;
